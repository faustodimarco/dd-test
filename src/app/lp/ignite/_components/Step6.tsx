...
export default function Step6() {
  ...
  const bgImage =
    STYLE_IMAGES[state.style as keyof typeof STYLE_IMAGES] ?? STYLE_IMAGES.realistic;

  const minutes = String(Math.floor(countdown / 60)).padStart(2, '0');
  const seconds = String(countdown % 60).padStart(2, '0');

  const characterName = state.name || 'Her';
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center px-4 py-8">
        ...
        >
          ...
          <p className="text-gray-400 text-sm text-center mb-4">
            {characterName} &middot; {state.gender} &middot; {state.style} &middot; Age {state.age}
          </p>
          ...
          <input
            ...
            className="w-full bg-white/10 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 mb-3"
          />
          ...
          <p className="text-gray-400 text-xs text-center">
            ...
            <span className="text-purple-400 font-semibold cursor-pointer">Log in &gt;</span>
          </p>
        </motion.div>
      </div>
    </div>
